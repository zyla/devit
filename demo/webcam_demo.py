...
def main(
    ...
    while True:
        ...
        output = draw_bounding_boxes(torch.as_tensor(image).permute(2, 0, 1), boxes,
                                                  labels=[f"{label_names[cid]} {score:.2f}" for cid, score in zip(pred_classes.tolist(), scores)], colors=colors)
        output_cv = cv2.cvtColor(output.numpy().transpose(1, 2, 0), cv2.COLOR_RGB2BGR)
        cv2.imshow("webcam", output_cv)
